<div class="card_ticket">
    ...
    // MISE A JOUR 14/10/2024 : ajout des heures , minutes et secondes dans la date
    ...
    // //insertion de la date du jour dans la table ticket
    // $requeteInsertionDate = $pdo->prepare("UPDATE ticket SET date_c = :dateDuJour WHERE id_ticket = :id_ticket");
    $prixMenu = $requetePrixMenu->fetch();		
    if (isset($ticketsBar) && is_array($ticketsBar) && !empty($ticketsBar)) {		 
    // Boucle pour chaque ticket en cours au bar
        foreach ($ticketsBar as $ticketBar) {
            // // MISE A JOUR 14/10/2024: Insertion de la date du jour dans la colonne date_c de la table ticket
            // $idticket_caisse = $ticketBar['id_ticket'];
            // $updateDate = $pdo->prepare("UPDATE ticket SET date_c = :dateDuJour WHERE id_ticket = :id_ticket");
            // $updateDate->bindParam(':dateDuJour', $dateDuJour);
            // $updateDate->bindParam(':id_ticket', $idticket_caisse);
            // $updateDate->execute();
            // ?>
            <div class="ticket">
